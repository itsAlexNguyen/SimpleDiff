...
import simplediff.gumtree.core.gen.SyntaxException;
...
public class JdtVisitor extends AbstractJdtVisitor {
  ...
  public boolean visit(MethodInvocation i) {
    if (i.getExpression() != null) {
      push(
          ...
          "",
          i.getExpression().getStartPosition(),
    ...
    if (i.arguments().size() > 0) {
      ...
      int length =
          ((ASTNode) i.arguments().get(i.arguments().size() - 1)).getStartPosition()
              + ((ASTNode) i.arguments().get(i.arguments().size() - 1)).getLength()
              - startPos;
  ...
  protected String getLabel(ASTNode n) {
    ...
    else if (n instanceof PrimitiveType) return n.toString();
    else if (n instanceof Modifier) return n.toString();
    else if (n instanceof StringLiteral) return ((StringLiteral) n).getEscapedValue();
    else if (n instanceof NumberLiteral) return ((NumberLiteral) n).getToken();
    else if (n instanceof CharacterLiteral) return ((CharacterLiteral) n).getEscapedValue();
    else if (n instanceof BooleanLiteral) return n.toString();
    else if (n instanceof TextElement) return n.toString();
    else return "";
  }
  ...
  public void postVisit(ASTNode n) {
    if (n instanceof TypeDeclaration) handlePostVisit((TypeDeclaration) n);
    else if (n instanceof InfixExpression) handlePostVisit((InfixExpression) n);
    else if (n instanceof Assignment) handlePostVisit((Assignment) n);
    else if (n instanceof PrefixExpression) handlePostVisit((PrefixExpression) n);
    else if (n instanceof PostfixExpression) handlePostVisit((PostfixExpression) n);
    else if (n instanceof ArrayCreation) handlePostVisit((ArrayCreation) n);
  ...
  private void handlePostVisit(TypeDeclaration d) {
    String label = "class";
    if (d.isInterface()) label = "interface";
    ...
    for (ITree c : t.getChildren()) {
      if (c.getType() != SIMPLE_NAME) index++;
      else break;
    }
