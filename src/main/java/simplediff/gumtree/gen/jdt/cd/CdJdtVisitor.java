...
package simplediff.gumtree.gen.jdt.cd;
...
import simplediff.gumtree.gen.jdt.AbstractJdtVisitor;

/**
 * Combination of two ChangeDistiller's AST visitors: JavaASTBodyTransformer and
 * JavaASTChangeDistillerVisitor. Modifications are labeled as "@Inria"
 *
 * <p>INRIA removed fNodeStack since it's inherited with the new
 */
...
public class CdJdtVisitor extends AbstractJdtVisitor {
  ...
  private boolean fnodeEmptyJavaDoc;
  private boolean fnodeInMethodDeclaration;
  ...
  public boolean visit(PrimitiveType node) {
    String visitName = "";
    if (fnodeInMethodDeclaration) {
      visitName += getCurrentParent().getLabel() + COLON_SPACE;
    }
    pushNode(node, visitName + node.getPrimitiveTypeCode().toString());
    return false;
  }
  ...
  public boolean visit(SimpleType node) {
    ...
    pushNode(node, visitName + node.getName().getFullyQualifiedName());
    return false;
  }
  ...
  public boolean visit(SingleVariableDeclaration node) {
    boolean isNotParam =
        !getCurrentParent().getLabel().equals(EntityType.PARAMETERS.toString()); // @inria
  ...
  public boolean visit(CatchClause node) {
    if (node.getException().getType() instanceof SimpleType) {
      pushNode(
          node, ((SimpleType) node.getException().getType()).getName().getFullyQualifiedName());
    } else {
  ...
  public void endVisit(Javadoc node) {
    if (!fnodeEmptyJavaDoc) {
      popNode();
    } else {
      fnodeEmptyJavaDoc = false;
    }
  }

  @Override
  public void endVisit(MethodDeclaration node) {
    fnodeInMethodDeclaration = false;
    popNode();
  }
