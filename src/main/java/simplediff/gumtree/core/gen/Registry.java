...
public abstract class Registry<K, C, A> {
  ...
  public C get(K key, Object... args) {
    ...
    if (factory != null) return factory.instantiate(args);
    return null;
  }

  public Factory<? extends C> getFactory(K key) {
    Entry entry = find(key);
    if (entry != null) return entry.factory;
    return null;
  }
  ...
  protected Entry findEntry(K key) {
    for (Entry e : entries) if (e.handle(key)) return e;
    return null;
  }

  public Entry findByClass(Class<? extends C> aClass) {
    for (Entry e : entries) if (e.clazz.equals(aClass)) return e;
    return null;
  }
  ...
  protected Factory<? extends C> defaultFactory(Class<? extends C> clazz, Class... signature) {
    try {
      ...
      throw new RuntimeException(
          String.format(
              "This is a static bug. Constructor %s(%s) not found",
              clazz.getName(), Arrays.toString(signature)),
          e);
    }
  }
  ...
  public abstract class Entry {
    ...
    protected Entry(
        String id, Class<? extends C> clazz, Factory<? extends C> factory, int priority) {
      this.id = id;
      this.clazz = clazz;
