...
public class Generators extends Registry<String, TreeGenerator, Register> {
  ...
  public TreeContext getTree(String file) throws UnsupportedOperationException, IOException {
    ...
    if (p == null) throw new UnsupportedOperationException("No generator found for file: " + file);
    return p.generateFrom().file(file);
  }

  public TreeContext getTree(String generator, String file)
      throws UnsupportedOperationException, IOException {
    for (Entry e : entries)
      if (e.id.equals(generator)) return e.instantiate(null).generateFrom().file(file);
