...
import simplediff.gumtree.core.io.TreeIoUtils.AbstractSerializer;
...
public final class ActionsIoUtils {
  ...
  public static ActionSerializer toText(TreeContext sctx, EditScript actions, MappingStore mappings)
      throws IOException {
  ...
  public static ActionSerializer toXml(TreeContext sctx, EditScript actions, MappingStore mappings)
      throws IOException {
    return new ActionSerializer(sctx, mappings, actions) {
  ...
  public static ActionSerializer toJson(TreeContext sctx, EditScript actions, MappingStore mappings)
      throws IOException {
    return new ActionSerializer(sctx, mappings, actions) {
  ...
  public abstract static class ActionSerializer extends AbstractSerializer {
    ...
    protected abstract ActionFormatter newFormatter(TreeContext ctx, Writer writer)
        throws Exception;
    ...
    public void writeTo(Writer writer) throws Exception {
      ...
      for (Action a : actions) {
        ...
        if (a instanceof Move) {
          ...
          ITree dst = a.getNode();
          if (dst.isRoot()) fmt.insertRoot((Insert) a, src);
          else
            fmt.insertAction(
                (Insert) a, src, dst.getParent(), dst.getParent().getChildPosition(dst));
        } else if (a instanceof Delete) {
          fmt.deleteAction((Delete) a, src);
        } else if (a instanceof TreeInsert) {
          ITree dst = a.getNode();
          fmt.insertTreeAction(
              (TreeInsert) a, src, dst.getParent(), dst.getParent().getChildPosition(dst));
  ...
  static class XmlFormatter implements ActionFormatter {
    ...
    @Override
    public void insertTreeAction(TreeInsert action, ITree node, ITree parent, int index)
        throws Exception {
      start(action, node);
      writer.writeAttribute("parent", parent.toString());
      writer.writeAttribute("at", Integer.toString(index));
  ...
  static class JsonFormatter implements ActionFormatter {
    ...
    @Override
    public void insertAction(Insert action, ITree node, ITree parent, int index)
        throws IOException {
      start(action, node);
