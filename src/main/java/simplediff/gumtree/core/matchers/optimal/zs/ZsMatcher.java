...
package simplediff.gumtree.core.matchers.optimal.zs;
...
public class ZsMatcher implements Matcher {
  ...
  private static class Implementation {
    ...
    private static ITree getFirstLeaf(ITree t) {
      ITree current = t;
      while (!current.isLeaf()) current = current.getChild(0);
    ...
    private double[][] computeTreeDist() {
      ...
      for (int i = 1; i < zsSrc.kr.length; i++)
        for (int j = 1; j < zsDst.kr.length; j++) forestDist(zsSrc.kr[i], zsDst.kr[j]);
    ...
    public void match() {
      ...
      while (!treePairs.isEmpty()) {
        ...
        if (!rootNodePair) forestDist(lastRow, lastCol);
        ...
        while ((row > firstRow) || (col > firstCol)) {
          if ((row > firstRow) && (forestDist[row - 1][col] + 1D == forestDist[row][col])) {
            ...
            if ((zsSrc.lld(row) - 1 == zsSrc.lld(lastRow) - 1)
              ...
              if (tSrc.getType() == tDst.getType()) mappings.addMapping(tSrc, tDst);
    ...
    private double getUpdateCost(ITree n1, ITree n2) {
      if (n1.getType() == n2.getType())
        if ("".equals(n1.getLabel()) || "".equals(n2.getLabel())) return 1D;
        else return 1D - StringMetrics.qGramsDistance().compare(n1.getLabel(), n2.getLabel());
