...
public class RtedAlgorithm {
  ...
  public void computeOptimalStrategy() {
    ...
    for (int v = 0; v < size1; v++) {
      ...
      for (int w = 0; w < size2; w++) {
        ...
        long[] mins = {
          leftMin, rightMin, heavyMin, Long.MAX_VALUE, revLeftMin, revRightMin, revHeavyMin
        };
    ...
    for (int i1 = 1; i1 <= i - ioff; i1++) {
      for (int j1 = 1; j1 <= j - joff; j1++) {
        if ((it1.info[POST2_LLD][i1 + ioff] == it1.info[POST2_LLD][i])
          ...
          db = forestdist[i1][j1 - 1] + costIns;
          dc =
              forestdist[it1.info[POST2_LLD][i1 + ioff] - 1 - ioff][
      ...
      for (int j1 = 1; j1 <= j - joff; j1++) {
        if ((it1.info[RPOST2_RLD][i1 + ioff] == it1.info[RPOST2_RLD][i])
          ...
          if (it1.info[POST2_LABEL][it1.info[RPOST2_POST][i1 + ioff]]
              != it2.info[POST2_LABEL][it2.info[RPOST2_POST][j1 + joff]]) u = costMatch;
          ...
          u =
              switched
                  ? deltaBit[it2.info[RPOST2_POST][j1 + joff]][it1.info[RPOST2_POST][i1 + ioff]]
                      * costMatch
          ...
          db = forestdist[i1][j1 - 1] + costIns;
          dc =
              forestdist[it1.info[RPOST2_RLD][i1 + ioff] - 1 - ioff][
  ...
  private void computePeriod(InfoTree it1, int aVp, int aNextVp, InfoTree it2, int aStrategy) {
    ...
    if (vpSize - nextVpSize == 1) {
      ...
      if (gSize == 1) {
        setDeltaValue(
            it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gPreorder], vpSize - 1, switched);
      } else {
        setDeltaValue(
            it1.info[PRE2_POST][vpPreorder], it2.info[PRE2_POST][gPreorder], t[1][0], switched);
      }
    }
    ...
    for (int i = gSize - 1; i >= 0; i--) {
      ...
      for (int kPrime = 1; kPrime <= k; kPrime++) {
        ...
        for (int j = jOfI; j >= 0; j--) {
          ...
          gijForestPreorder =
              aStrategy == LEFT ? ij[i][j] : it2.info[POST2_PRE][gTreeSize - 1 - ij[i][j]];
          ...
          match +=
              switched
                  ? delta[it2.info[PRE2_POST][gijForestPreorder]][
                      it1.info[PRE2_POST][fForestPreorderKPrime]]
                  : delta[it1.info[PRE2_POST][fForestPreorderKPrime]][
          ...
          // fill S table
          s[kPrime - 1][j] =
              (deleteFromLeft < deleteFromRight)
      ...
      // early
      t[i] = (realStrategy == BOTH && aStrategy == LEFT) ? s[k - 1 - 1].clone() : s[k - 1].clone();
  ...
  /**
   * Compute the minimal edit mapping between two trees. There might be multiple minimal edit
   * mappings. This function computes only one of them.
   *
   * <p>The first step of this function is to compute the tree edit distance. Based on the tree
   * distance matrix the mapping is computed.
   *
   * @return all pairs (ted1.node,ted2.node) of the minimal edit mapping. Each element in the
   *     collection is an integer array A of size 2, where A[0]=ted1.node is the postorderID
   *     (starting with 1) of the nodes in ted1 and A[1]=ted2.node is the postorderID in ted2. The
  ...
  private void forestDist(
      InfoTree ted1, InfoTree ted2, int i, int j, double[][] treedist, double[][] forestdist) {
    ...
    for (int di = ted1.getInfo(POST2_LLD, i - 1) + 1; di <= i; di++) {
      forestdist[di][ted2.getInfo(POST2_LLD, j - 1) + 1 - 1] =
          forestdist[di - 1][ted2.getInfo(POST2_LLD, j - 1) + 1 - 1] + costDel;
      for (int dj = ted2.getInfo(POST2_LLD, j - 1) + 1; dj <= j; dj++) {
        forestdist[ted1.getInfo(POST2_LLD, i - 1) + 1 - 1][dj] =
            forestdist[ted1.getInfo(POST2_LLD, i - 1) + 1 - 1][dj - 1] + costIns;
        ...
        if ((ted1.getInfo(POST2_LLD, di - 1) == ted1.getInfo(POST2_LLD, i - 1))
          ...
          forestdist[di][dj] =
              Math.min(
                  Math.min(forestdist[di - 1][dj] + costDel, forestdist[di][dj - 1] + costIns),
                  forestdist[ted1.getInfo(POST2_LLD, di - 1) + 1 - 1][
