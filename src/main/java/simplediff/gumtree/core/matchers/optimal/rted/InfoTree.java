...
public class InfoTree {
  ...
  public static final byte POST2_STRATEGY =
      14; // strategy for Demaine (is there sth on the left/right of the heavy node)
  ...
  public boolean[][]
      nodeType; // store the type of a node: for every node stores three boolean values (L, R, H)
  ...
  // remembers if the trees order was switched during the recursion (in comparison with the order of
  // input trees)
   ...
   *
   * <p>At this point the given tree is traversed once, but there is a loop over current nodes
   * children to assign them their parents.
   *
   * @param aT
   * @param postorder
   * @return
  ...
  private void postTraversalProcessing() {
    ...
    for (i = 0; i < leafCount; i++) {
      ...
      while (parentR > -1 && info[RPOST2_MIN_RKR][parentR] == -1) {
        ...
        if (parentR > -1) {
          parentR =
              treeSize - 1 - info[POST2_PRE][parentR]; // if parent exists get its rev. postorder
        }
      }
    }
  }
