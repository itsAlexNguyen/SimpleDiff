...
import simplediff.gumtree.core.utils.HungarianAlgorithm;

public class HungarianSubtreeMatcher extends AbstractSubtreeMatcher implements Matcher {
  @Override
  public MappingStore match(ITree src, ITree dst, MappingStore mappings) {
    HungarianSubtreeMatcher.Implementation impl =
        new HungarianSubtreeMatcher.Implementation(src, dst, mappings);
    impl.match();
    ...
    public void filterMappings(MultiMappingStore multiMappings) {
      ...
      for (ITree src : multiMappings.allMappedSrcs())
        if (multiMappings.isSrcUnique(src))
          ...
          for (ITree asrc : asrcs) for (ITree adst : adsts) ambiguous.addMapping(asrc, adst);
      ...
      for (MultiMappingStore ambiguous : ambiguousList) {
        ...
        for (int i = 0; i < solutions.length; i++) {
          ...
          if (dstIdx != -1) mappings.addMappingRecursively(lstSrcs.get(i), lstDsts.get(dstIdx));
        }
      }
    }
