...
import simplediff.gumtree.core.matchers.optimal.zs.ZsMatcher;
...
public abstract class AbstractBottomUpMatcher {
  ...
  public static double SIM_THRESHOLD = Double.parseDouble(System.getProperty("gt.bum.smt", "0.5"));
  ...
  protected abstract static class Implementation {
    ...
    protected List<ITree> getDstCandidates(ITree src) {
      ...
      for (ITree c : src.getDescendants()) {
        if (mappings.isSrcMapped(c)) seeds.add(mappings.getDstForSrc(c));
      }
      ...
      for (ITree seed : seeds) {
        while (seed.getParent() != null) {
          ...
          visited.add(parent);
          if (parent.getType() == src.getType()
              && !(mappings.isDstMapped(parent) || parent.isRoot())) candidates.add(parent);
          seed = parent;
        }
      }
    ...
    protected void lastChanceMatch(ITree src, ITree dst) {
      if (src.getMetrics().size < AbstractBottomUpMatcher.SIZE_THRESHOLD
        ...
        for (Mapping candidate : zsMappings) {
          ...
          if (mappings.isMappingAllowed(srcCand, dstCand)) mappings.addMapping(srcCand, dstCand);
        }
      }
    }
  }
