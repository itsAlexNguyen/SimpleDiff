...
package simplediff.gumtree.core.matchers.heuristic.cd;
...
public class ChangeDistillerBottomUpMatcher implements Matcher {
  ...
  public static final double STRUCT_SIM_THRESHOLD_1 =
      Double.parseDouble(System.getProperty("gt.cd.ssim1", "0.6"));

  public static final double STRUCT_SIM_THRESHOLD_2 =
      Double.parseDouble(System.getProperty("gt.cd.ssim2", "0.4"));

  public static final int MAX_NUMBER_OF_LEAVES =
      Integer.parseInt(System.getProperty("gt.cd.ml", "4"));
  ...
  private static class Implementation {
          ...
          if (mappings.isMappingAllowed(currentSrcTree, currentDstTree)
            ...
            if ((numberOfLeaves > MAX_NUMBER_OF_LEAVES && similarity >= STRUCT_SIM_THRESHOLD_1)
                || (numberOfLeaves <= MAX_NUMBER_OF_LEAVES
                    && similarity >= STRUCT_SIM_THRESHOLD_2)) {
    ...
    private int numberOfLeaves(ITree root) {
      ...
      for (ITree tree : root.getDescendants()) if (tree.isLeaf()) numberOfLeaves++;
