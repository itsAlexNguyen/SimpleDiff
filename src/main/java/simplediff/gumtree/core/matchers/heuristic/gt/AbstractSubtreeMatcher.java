...
import simplediff.gumtree.core.matchers.MultiMappingStore;
...
public abstract class AbstractSubtreeMatcher {
  ...
  protected abstract static class Implementation {
    ...
    private void popLarger(PriorityTreeList srcTrees, PriorityTreeList dstTrees) {
      if (srcTrees.peekHeight() > dstTrees.peekHeight()) srcTrees.open();
      else dstTrees.open();
    }
    ...
    public void match() {
      ...
      while (srcTrees.peekHeight() != -1 && dstTrees.peekHeight() != -1) {
        ...
        for (int i = 0; i < marksForSrcTrees.length; i++)
          if (marksForSrcTrees[i] == false) srcTrees.open(currentHeightSrcTrees.get(i));
        for (int j = 0; j < marksForDstTrees.length; j++)
          if (marksForDstTrees[j] == false) dstTrees.open(currentHeightDstTrees.get(j));
    ...
    protected double sim(ITree src, ITree dst) {
      double jaccard =
          SimilarityMetrics.jaccardSimilarity(src.getParent(), dst.getParent(), mappings);
      ...
      double po =
          1D
              - ((double) Math.abs(src.getMetrics().position - dst.getMetrics().position)
    ...
    protected void retainBestMapping(
        List<Mapping> mappingList, Set<ITree> srcIgnored, Set<ITree> dstIgnored) {
