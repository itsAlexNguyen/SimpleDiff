  ...
  protected static class Implementation extends AbstractSubtreeMatcher.Implementation {
    ...
    private List<Mapping> fromClique(Pair<List<ITree>, List<ITree>> clique) {
      ...
      for (ITree src : clique.first)
        for (ITree dst : clique.first) cliqueAsMappings.add(new Mapping(src, dst));
    ...
    private class CliqueComparator implements Comparator<Pair<List<ITree>, List<ITree>>> {
      ...
      public int compare(Pair<List<ITree>, List<ITree>> l1, Pair<List<ITree>, List<ITree>> l2) {
        ...
        if (minDepth1 != minDepth2) return -1 * Integer.compare(minDepth1, minDepth2);
        else {
          int size1 = size(l1);
          int size2 = size(l2);
      ...
      private int minDepth(Pair<List<ITree>, List<ITree>> trees) {
        ...
        for (ITree t : trees.first) if (depth > t.getMetrics().depth) depth = t.getMetrics().depth;
        for (ITree t : trees.second) if (depth > t.getMetrics().depth) depth = t.getMetrics().depth;
        return depth;
      }
    ...
    private class MappingComparator implements Comparator<Mapping> {
      ...
      public MappingComparator(List<Mapping> mappings) {
        for (Mapping mapping : mappings) simMap.put(mapping, sims(mapping.first, mapping.second));
      }
      ...
      protected double jaccardSimilarity(ITree src, ITree dst) {
        ...
        double den =
            (double) srcDescendants.get(src).size() + (double) dstDescendants.get(dst).size() - num;
        return num / den;
      }
    }
  }
