...
public class GreedySubtreeMatcher extends AbstractSubtreeMatcher implements Matcher {
  ...
  public MappingStore match(ITree src, ITree dst, MappingStore mappings) {
    GreedySubtreeMatcher.Implementation impl =
        new GreedySubtreeMatcher.Implementation(src, dst, mappings);
    impl.match();
  ...
  protected static class Implementation extends AbstractSubtreeMatcher.Implementation {
    ...
    public void filterMappings(MultiMappingStore multiMappings) {
      ...
      for (ITree src : multiMappings.allMappedSrcs()) {
        if (multiMappings.isSrcUnique(src))
          ...
          for (ITree asrc : asrcs)
            for (ITree adst : adsts) ambiguousList.add(new Mapping(asrc, adst));
      ...
      Collections.sort(
          ambiguousList, new SiblingsMappingComparator(ambiguousList, mappings, getMaxTreeSize()));
