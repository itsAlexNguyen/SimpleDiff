...
package simplediff.gumtree.core.matchers.heuristic;
...
import simplediff.gumtree.core.matchers.SimilarityMetrics;
import simplediff.gumtree.core.tree.ITree;
import simplediff.gumtree.core.tree.Type;

/**
 * Match the nodes using a bottom-up approach. It browse the nodes of the source and destination
 * trees using a post-order traversal, testing if the two selected trees might be mapped. The two
 * trees are mapped if they are mappable and have a dice coefficient greater than SIM_THRESHOLD.
 * Whenever two trees are mapped a exact ZS algorithm is applied to look to possibly forgotten
 * nodes.
 */
public class XyBottomUpMatcher implements Matcher {

  private static final double SIM_THRESHOLD =
      Double.parseDouble(System.getProperty("gt.xym.sim", "0.5"));
  ...
  private static class Implementation {
    ...
    private void lastChanceMatch(ITree src, ITree dst) {
      ...
      for (Type t : srcKinds.keySet())
        if (dstKinds.get(t) != null
            && srcKinds.get(t).size() == dstKinds.get(t).size()
