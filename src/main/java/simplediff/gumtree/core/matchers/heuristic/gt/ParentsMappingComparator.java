...
public final class ParentsMappingComparator extends AbstractMappingComparator {
  ...
  public ParentsMappingComparator(
      List<Mapping> ambiguousMappings, MappingStore mappings, int maxTreeSize) {
    super(ambiguousMappings, mappings, maxTreeSize);
    for (Mapping ambiguousMapping : ambiguousMappings)
      similarities.put(
          ambiguousMapping, similarity(ambiguousMapping.first, ambiguousMapping.second));
  }

  @Override
  protected double similarity(ITree src, ITree dst) {
    return 100D * parentsJaccardSimilarity(src, dst)
        + 10D * posInParentSimilarity(src, dst)
        + numberingSimilarity(src, dst);
  }
  ...
  protected double parentsJaccardSimilarity(ITree src, ITree dst) {
    ...
    double numerator =
        SequenceAlgorithms.longestCommonSubsequenceWithTypeAndLabel(srcParents, dstParents)
            .size();
