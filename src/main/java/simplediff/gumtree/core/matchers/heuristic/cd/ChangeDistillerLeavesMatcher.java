...
public class ChangeDistillerLeavesMatcher implements Matcher {
  ...
  public static final double LABEL_SIM_THRESHOLD =
      Double.parseDouble(System.getProperty("gt.cd.lsim", "0.5"));
  ...
  private static class Implementation {
    ...
    public void match() {
      ...
      for (Iterator<ITree> srcLeaves = TreeUtils.leafIterator(TreeUtils.postOrderIterator(src));
          srcLeaves.hasNext(); ) {
        ITree srcLeaf = srcLeaves.next();
        for (ITree dstLeaf : dstLeaves) {
          if (mappings.isMappingAllowed(srcLeaf, dstLeaf)) {
            double sim =
                StringMetrics.qGramsDistance().compare(srcLeaf.getLabel(), dstLeaf.getLabel());
    ...
    public List<ITree> retainLeaves(List<ITree> trees) {
      ...
      while (treeIterator.hasNext()) {
        ...
        if (!tree.isLeaf()) treeIterator.remove();
      }
      return trees;
    }
  }
