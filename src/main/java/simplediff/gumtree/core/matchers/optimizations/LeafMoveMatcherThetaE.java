...
public class LeafMoveMatcherThetaE implements Matcher {
  ...
  private static class Implementation {
    ...
    private void thetaE() {
      ...
      while (!workList.isEmpty()) {
        ...
        for (Mapping entry : changeMap) {
          ...
          if (!entry.first.getLabel().equals(entry.second.getLabel())
              && entry.first.isLeaf()
              && entry.second.isLeaf()) {
    ...
    private void reevaluateLeaves(
        ITree firstParent, ITree secondParent, Mapping pair, List<Mapping> changeMap) {
