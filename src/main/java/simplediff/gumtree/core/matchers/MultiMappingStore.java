...
public class MultiMappingStore implements Iterable<Mapping> {
  ...
  public MultiMappingStore(Set<Mapping> mappings) {
    this();
    for (Mapping m : mappings) addMapping(m.first, m.second);
  }
  ...
  public Set<Mapping> getMappings() {
    ...
    for (ITree src : srcToDsts.keySet())
      for (ITree dst : srcToDsts.get(src)) mappings.add(new Mapping(src, dst));
    return mappings;
  }

  public void addMapping(ITree src, ITree dst) {
    if (!srcToDsts.containsKey(src)) srcToDsts.put(src, new HashSet<ITree>());
    srcToDsts.get(src).add(dst);
    if (!dstToSrcs.containsKey(dst)) dstToSrcs.put(dst, new HashSet<ITree>());
