...
import simplediff.gumtree.core.io.TreeIoUtils.MetadataSerializer;
import simplediff.gumtree.core.io.TreeIoUtils.MetadataUnserializer;
import simplediff.gumtree.core.io.TreeIoUtils.TreeFormatter;
...
public class TreeContext {
  ...
  /**
   * Get a global metadata. There is no way to know if the metadata is really null or does not
   * exists.
   *
  ...
  /**
   * Get a local metadata, if available. Otherwise get a global metadata. There is no way to know if
   * the metadata is really null or does not exists.
   *
   * @param key of metadata
   * @return the metadata or null if not found
   */
  public Object getMetadata(ITree node, String key) {
    Object metadata;
    if (node == null || (metadata = node.getMetadata(key)) == null) return getMetadata(key);
    return metadata;
  }
  ...
  public Object setMetadata(ITree node, String key, Object value) {
    if (node == null) return setMetadata(key, value);
    else {
      Object res = node.setMetadata(key, value);
      if (res == null) return getMetadata(key);
      return res;
    }
  }
  ...
  public TreeContext export(String... name) {
    for (String n : name) serializers.add(n, x -> x.toString());
    return this;
  }

  public TreeContext
      deriveTree() { // FIXME Should we refactor TreeContext class to allow shared metadata etc ...
  ...
  /**
   * Get an iterator on local and global metadata. To only get local metadata, simply use :
   * `node.getMetadata()`
   */
  public Iterator<Entry<String, Object>> getMetadata(ITree node) {
    if (node == null) return getMetadata();
  ...
  public static class Marshallers<E> {
    ...
    public void add(String name, E serializer) {
      if (!valid_id
          .matcher(name)
          .matches()) // TODO I definitely don't like this rule, we should think twice
  ...
  public static class MetadataSerializers extends Marshallers<MetadataSerializer> {
    ...
    public void serialize(TreeFormatter formatter, String key, Object value) throws Exception {
      ...
      if (s != null) formatter.serializeAttribute(key, s.toString(value));
    }
  }
  ...
  public static class MetadataUnserializers extends Marshallers<MetadataUnserializer> {
    ...
    public void load(ITree tree, String key, String value) throws Exception {
      ...
      if (s != null) {
        if (key.equals("pos")) tree.setPos(Integer.parseInt(value));
        else if (key.equals("length")) tree.setLength(Integer.parseInt(value));
