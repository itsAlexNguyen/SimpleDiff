...
public final class TreeUtils {
  ...
  private static void preOrder(ITree tree, List<ITree> trees) {
    trees.add(tree);
    if (!tree.isLeaf()) for (ITree c : tree.getChildren()) preOrder(c, trees);
  }
    ...
    return new Iterator<ITree>() {
      ...
      public ITree next() {
        while (!fifo.isEmpty()) {
          ...
          if (it.hasNext()) {
            ITree item = it.next();
            if (!it.hasNext()) fifo.removeFirst();
            addLasts(item);
            return item;
          }
        }
      ...
      private void addLasts(ITree item) {
        ...
        if (!children.isEmpty()) fifo.addLast(children.iterator());
      }
  ...
  public static Iterator<ITree> postOrderIterator(final ITree tree) {
    return new Iterator<ITree>() {
      ...
      ITree selectNextChild(Iterator<ITree> it) {
        if (!it.hasNext()) return stack.pop().first;
        ITree item = it.next();
  ...
  public static Iterator<ITree> preOrderIterator(ITree tree) {
    return new Iterator<ITree>() {
      ...
      private void push(ITree tree) {
        if (!tree.isLeaf()) stack.push(tree.getChildren().iterator());
      }
