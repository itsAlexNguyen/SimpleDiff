...
public interface ITree {
  ...
  /**
   * @return all the descendants (children, children of children, etc.) of the tree, using a
   *     pre-order.
   */
  ...
  /**
   * Set the parent of this node. The parent will have this node in its children list, at the last
   * position.
   *
  ...
  default int positionInParent() {
    ITree p = getParent();
    if (p == null) return -1;
    else return p.getChildren().indexOf(this);
  }
  ...
  /**
   * Indicates whether or not this node and its descendants are isomorphic to the node given in
   * parameter and its descendants (which must not be null). This test fails fast.
   */
  default boolean isIsomorphicTo(ITree tree) {
    if (!hasSameTypeAndLabel(tree)) return false;

    if (getChildren().size() != tree.getChildren().size()) return false;
  ...
  /**
   * Indicates whether or not this node and its descendants are isostructural (isomorphism without
   * labels) to the node given in parameter and its descendants (which must not be null). This test
   * fails fast.
   */
  default boolean isIsoStructuralTo(ITree tree) {
    if (this.getType() != tree.getType()) return false;

    if (getChildren().size() != tree.getChildren().size()) return false;

    for (int i = 0; i < getChildren().size(); i++) {
      boolean isChildrenStructural = getChild(i).isIsoStructuralTo(tree.getChild(i));
      if (!isChildrenStructural) return false;
    }
