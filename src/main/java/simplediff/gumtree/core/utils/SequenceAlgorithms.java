...
public final class SequenceAlgorithms {
   ...
   *
   * @return a list of size 2 int arrays that corresponds to match of index in sequence 1 to index
   *     in sequence 2.
   */
  public static List<int[]> longestCommonSubsequence(String s0, String s1) {
    ...
    for (int i = 0; i < s0.length(); i++)
      for (int j = 0; j < s1.length(); j++)
        if (s0.charAt(i) == (s1.charAt(j))) lengths[i + 1][j + 1] = lengths[i][j] + 1;
   ...
   *
   * @return the hunks as a list of int arrays of size 4 with start index and end index of sequence
   *     1 and corresponding start index and end index in sequence 2.
   */
  ...
  /**
   * Returns the longest common subsequence between the two list of nodes. This version use type and
   * label to ensure equality.
   *
   * @see ITree#hasSameTypeAndLabel(ITree)
   * @return a list of size 2 int arrays that corresponds to match of index in sequence 1 to index
   *     in sequence 2.
   */
  public static List<int[]> longestCommonSubsequenceWithTypeAndLabel(
      List<ITree> s0, List<ITree> s1) {
    int[][] lengths = new int[s0.size() + 1][s1.size() + 1];
    for (int i = 0; i < s0.size(); i++)
      for (int j = 0; j < s1.size(); j++)
        if (s0.get(i).hasSameTypeAndLabel(s1.get(j))) lengths[i + 1][j + 1] = lengths[i][j] + 1;
        else lengths[i + 1][j + 1] = Math.max(lengths[i + 1][j], lengths[i][j + 1]);
   ...
   *     in sequence 2.
   */
  public static List<int[]> longestCommonSubsequenceWithIsomorphism(
      List<ITree> s0, List<ITree> s1) {
    int[][] lengths = new int[s0.size() + 1][s1.size() + 1];
    for (int i = 0; i < s0.size(); i++)
      for (int j = 0; j < s1.size(); j++)
        if (s0.get(i).isIsomorphicTo(s1.get(j))) lengths[i + 1][j + 1] = lengths[i][j] + 1;
        else lengths[i + 1][j + 1] = Math.max(lengths[i + 1][j], lengths[i][j + 1]);
   ...
   *     in sequence 2.
   */
  public static List<int[]> longestCommonSubsequenceWithIsostructure(
      List<ITree> s0, List<ITree> s1) {
    int[][] lengths = new int[s0.size() + 1][s1.size() + 1];
    for (int i = 0; i < s0.size(); i++)
      for (int j = 0; j < s1.size(); j++)
        if (s0.get(i).isIsoStructuralTo(s1.get(j))) lengths[i + 1][j + 1] = lengths[i][j] + 1;
        else lengths[i + 1][j + 1] = Math.max(lengths[i + 1][j], lengths[i][j + 1]);
