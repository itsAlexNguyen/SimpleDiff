...
public class ChawatheScriptGenerator implements EditScriptGenerator {
  ...
  public void initWith(MappingStore ms) {
    ...
    for (Mapping m : origMappings) cpyMappings.addMapping(origToCopy.get(m.first), m.second);
  }
  ...
  public EditScript generate() {
    ...
    List<ITree> topLevelSrc = origSrc.getChildren();
    List<ITree> topLevelDst = origDst.getChildren();
    ...
    for (ITree x : bfsDst) {
      ...
      if (x.getType().name.equals("PackageDeclaration")
          || y.getType().name.equals("PackageDeclaration")) {
        System.out.println("Package change");
      }

      if (x.getType().name.equals("ImportDeclaration")) {
        System.out.println("Import change");
      }
  ...
  private void alignChildren(ITree w, ITree x) {
    ...
    for (ITree c : w.getChildren())
      if (cpyMappings.isSrcMapped(c))
        if (x.getChildren().contains(cpyMappings.getDstForSrc(c))) s1.add(c);

    List<ITree> s2 = new ArrayList<>();
    for (ITree c : x.getChildren())
      if (cpyMappings.isDstMapped(c))
        if (w.getChildren().contains(cpyMappings.getSrcForDst(c))) s2.add(c);
