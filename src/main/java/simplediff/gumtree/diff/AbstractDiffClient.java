...
import simplediff.gumtree.client.Client;
...
public abstract class AbstractDiffClient<O extends AbstractDiffClient.Options> extends Client {
  ...
  public AbstractDiffClient(String[] args) {
    ...
    if (args.length < 2) throw new Option.OptionException("arguments required." + SYNTAX, opts);
  ...
  protected Matcher getMatcher() {
    ...
    matcher = (opts.matcher == null) ? matchers.getMatcher() : matchers.getMatcher(opts.matcher);
    return matcher;
  }
  ...
  protected TreeContext getSrcTreeContext() {
    if (src == null) src = getTreeContext(opts.src);
    return src;
  }

  protected TreeContext getDstTreeContext() {
    if (dst == null) dst = getTreeContext(opts.dst);
    return dst;
  }
  ...
  protected TreeContext getTreeContext(String file) {
    try {
      TreeContext t;
      if (opts.generator == null) t = Generators.getInstance().getTree(file);
      else t = Generators.getInstance().getTree(opts.generator, file);
      return t;
    } catch (IOException e) {
      e.printStackTrace();
    }
