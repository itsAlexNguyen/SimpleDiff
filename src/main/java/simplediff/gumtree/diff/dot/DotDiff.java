...
package simplediff.gumtree.diff.dot;
...
public final class DotDiff extends AbstractDiffClient<AbstractDiffClient.Options> {
  ...
  private void writeTree(TreeContext context, Writer writer, MappingStore mappings)
      throws Exception {
    for (ITree tree : context.getRoot().preOrder()) {
      String fillColor = "red";
      if (mappings.isSrcMapped(tree) || mappings.isDstMapped(tree)) fillColor = "blue";
      writer.write(
          String.format(
              "%s [label=\"%s\", color=%s];\n",
