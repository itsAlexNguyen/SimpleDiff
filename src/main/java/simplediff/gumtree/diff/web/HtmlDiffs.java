...
import simplediff.gumtree.core.actions.ITreeClassifier;
import simplediff.gumtree.core.tree.ITree;
import simplediff.gumtree.core.tree.TreeContext;
import simplediff.gumtree.core.utils.SequenceAlgorithms;

public final class HtmlDiffs {

  private static final String SRC_MV_SPAN =
      "<span class=\"%s\" id=\"move-src-%d\" data-title=\"%s\">";
  private static final String DST_MV_SPAN =
      "<span class=\"%s\" id=\"move-dst-%d\" data-title=\"%s\">";
  ...
  public void produce() throws IOException {
    ...
    for (ITree t : diff.src.getRoot().preOrder()) {
      if (c.getMovedSrcs().contains(t)) {
        ...
        ltags.addTags(
            t.getPos(),
            String.format(SRC_MV_SPAN, "token mv", mId++, tooltip(diff.src, t)),
            t.getEndPos(),
            END_SPAN);
      }
      if (c.getUpdatedSrcs().contains(t)) {
        ...
        ltags.addTags(
            t.getPos(),
            String.format(SRC_MV_SPAN, "token upd", mId++, tooltip(diff.src, t)),
            t.getEndPos(),
            END_SPAN);
        List<int[]> hunks =
            SequenceAlgorithms.hunks(t.getLabel(), diff.mappings.getDstForSrc(t).getLabel());
        for (int[] hunk : hunks)
      ...
      if (c.getDeletedSrcs().contains(t)) {
        ...
        ltags.addTags(
            t.getPos(),
            String.format(ADD_DEL_SPAN, "token del", tooltip(diff.src, t)),
            t.getEndPos(),
            END_SPAN);
      }
    }
    ...
    for (ITree t : diff.dst.getRoot().preOrder()) {
      if (c.getMovedDsts().contains(t)) {
        ...
        rtags.addTags(
            t.getPos(),
            String.format(DST_MV_SPAN, "token mv", dId, tooltip(diff.dst, t)),
            t.getEndPos(),
            END_SPAN);
      }
      if (c.getUpdatedDsts().contains(t)) {
        ...
        rtags.addTags(
            t.getPos(),
            String.format(DST_MV_SPAN, "token upd", dId, tooltip(diff.dst, t)),
            t.getEndPos(),
            END_SPAN);
        List<int[]> hunks =
            SequenceAlgorithms.hunks(diff.mappings.getSrcForDst(t).getLabel(), t.getLabel());
        for (int[] hunk : hunks)
      ...
      if (c.getInsertedDsts().contains(t)) {
        ...
        rtags.addTags(
            t.getPos(),
            String.format(ADD_DEL_SPAN, "token add", tooltip(diff.dst, t)),
            t.getEndPos(),
            END_SPAN);
      }
    }
    ...
    BufferedReader r = Files.newBufferedReader(fSrc.toPath(), StandardCharsets.UTF_8);
    int cursor = 0;
    ...
    r = Files.newBufferedReader(fDst.toPath(), StandardCharsets.UTF_8);
    cursor = 0;
