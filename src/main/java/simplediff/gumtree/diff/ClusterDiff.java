...
import simplediff.gumtree.core.actions.ActionClusterFinder;
...
@Register(
    name = "cluster",
    description = "Extract action clusters",
...
public class ClusterDiff extends AbstractDiffClient<AbstractDiffClient.Options> {
  ...
  public void run() {
    ...
    ActionClusterFinder f =
        new ActionClusterFinder(getSrcTreeContext(), getDstTreeContext(), actions);
    for (Set<Action> cluster : f.getClusters()) {
      System.out.println("New cluster:");
      System.out.println(f.getClusterLabel(cluster));
      System.out.println("------------");
      for (Action a : cluster) System.out.println(a.toString());
      System.out.println();
    }
  }
