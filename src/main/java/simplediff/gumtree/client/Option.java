...
public abstract class Option {
  ...
  private Option(String key, String text) {
    this(key, text, 0);
  }

  protected Option(String key, String text, int params) {
    this.key = key;
  ...
  private static String[] processCommandLine(
      String[] args, Option[] availableOptions, Context ctx) {
          ...
          while (currentParam < nbParams) {
            try {
              ...
              throw new OptionException(
                  String.format("Option '%s' expects more parameters, using null", arg), ctx);
            }
          }
  ...
  static void displayOptions(PrintStream out, Context ctx) {
  ...
  public String formatHelpText() {
    return String.format(
        "%s%s\t%s", key, (paramCount > 0 ? " <" + paramCount + ">" : ""), description);
  }
  ...
  public interface Context {
     ...
     * @return the combined options set
     */
